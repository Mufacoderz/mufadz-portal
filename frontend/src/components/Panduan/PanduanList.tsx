            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6 mb-6">
                {homeSections.map((feature) => (
                    <button
                        ...
                        className="group bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700 hover:shadow-xl hover:border-blue-400 hover:-translate-y-1 transition-all duration-300 flex flex-col gap-4 text-left"
                    >
                        <div className="flex items-center justify-start gap-3">
                            {feature.icon}
                            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-300">{feature.title}</h3>
                        </div>
                        <p className="text-gray-600 dark:text-gray-300">{feature.desc}</p>
                        <span className="mt-auto text-blue-500 dark:text-blue-300 font-medium group-hover:underline">Buka →</span>
                    </button>
                ))}
            </div>
