        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-blue-100 dark:border-gray-700 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-base text-gray-800 dark:text-white">Reminder & Todo</h2>
                    {todo.length > 0 && (
            ...
            {/* Form tambah */}
            {showForm && (
                <div className="px-6 py-4 bg-blue-50/60 dark:bg-blue-900/10 border-b border-blue-100 dark:border-gray-700 space-y-3">
                    <input
                        type="text"
                    ...
                    <button
                        ...
                        className="w-full bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors disabled:opacity-60 shadow-sm"
                    >
            ...
            <div className="px-6 py-4">
                ...
                {loading && (
                    <div className="flex items-center justify-center py-8 gap-2 text-gray-400">
                        ...
                        <span className="text-sm">Memuat reminder...</span>
                    </div>
                )}
                ...
                {!loading && reminders.length === 0 && (
                    <div className="text-center py-10">
                        <div className="w-14 h-14 rounded-2xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center mx-auto mb-3">
                            <svg className="w-7 h-7 text-blue-300 dark:text-blue-600" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                            </svg>
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Belum ada reminder</p>
                        <p className="text-gray-400 dark:text-gray-600 text-xs mt-1">Tambahkan reminder untuk aktivitasmu</p>
                    </div>
                )}
                ...
                {done.length > 0 && (
                    <div className="mt-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-xs text-gray-400 dark:text-gray-500 font-medium px-2">Selesai ({done.length})</p>
                            <div className="flex-1 h-px bg-gray-100 dark:bg-gray-700" />
                        </div>
                        <div className="space-y-2">
                            {done.map((r) => (
...
function ReminderItem({
    ...
    return (
        <div className={`flex items-start gap-3 p-3.5 rounded-xl border transition-all duration-200 group
            ${reminder.is_done
                ? "bg-gray-50 dark:bg-gray-700/30 border-gray-100 dark:border-gray-700/50 opacity-50"
                : isOverdue
                    ? "bg-red-50/50 dark:bg-red-900/10 border-red-100 dark:border-red-900/30"
                    : "bg-gray-50 dark:bg-gray-700/40 border-gray-100 dark:border-gray-700 hover:border-blue-200 dark:hover:border-blue-800 hover:bg-blue-50/30 dark:hover:bg-blue-900/10"
            }`}
        >
            {/* Checkbox */}
            <button
            ...
            <div className="flex-1 min-w-0">
                <p className={`text-sm font-semibold ${reminder.is_done ? "line-through text-gray-400" : "text-gray-700 dark:text-gray-200"}`}>
                    {reminder.title}
                </p>
                {reminder.note && (
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 leading-relaxed">{reminder.note}</p>
                )}
                {formattedTime && (
                    <div className={`flex items-center gap-1 mt-1.5 text-xs ${isOverdue ? "text-red-500" : "text-gray-400 dark:text-gray-500"}`}>
